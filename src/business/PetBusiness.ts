...
export class PetBusiness {
    ...
    public async deletarPet(id: Number): Promise<any> {
        try {
            const idExiste = await this.verificarIdExistente(id);
            if (!idExiste) {
                throw new Error("Falha ao encontrar pet: Id vinculado a nenhum pet!");
            } else {
                await this.petData.deletarPetNoBancoDeDados(id);
            }
        } catch (error: any) {
            throw new Error(error.message);
        }
